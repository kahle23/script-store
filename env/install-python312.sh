...
DEFAULT_ADD_TO_PATH="false" # 新增：默认不添加到环境变量
...
ADD_TO_PATH="${DEFAULT_ADD_TO_PATH}" # 新增参数变量
...
if [[ ! "${PYTHON_VERSION}" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    echo "错误: 无效的版本号格式 '${PYTHON_VERSION}'，应为 X.X.X 格式"
    exit 1
fi
...
echo "安装目录: ${PYTHON_INSTALL_DIR}"
echo "添加到环境变量: ${ADD_TO_PATH}"
echo "软件包缓存: ${PKG_DOWNLOAD_BASE}"
echo "=========================="
...
sudo mkdir -p "${PKG_DOWNLOAD_BASE}"
sudo chown -R $(whoami):$(whoami) "${INSTALL_BASE}"
sudo chown -R $(whoami):$(whoami) "${PKG_DOWNLOAD_BASE}"

# 3. 下载Python源码（使用国内加速镜像）
cd "${PKG_DOWNLOAD_BASE}"
if [ ! -f "Python-${PYTHON_VERSION}.tgz" ]; then
    echo "步骤 3/8: 下载 Python ${PYTHON_VERSION} 源码包..."
...
# 6. 编译安装
echo "步骤 6/8: 编译安装Python (可能需要较长时间)..."
make -j $(nproc)
make altinstall

# 7. 根据参数选择是否配置环境变量
if [ "${ADD_TO_PATH}" = "true" ]; then
    echo "步骤 7/8: 配置系统环境变量..."
...
EOF
    ...
    echo "步骤 7/8: 跳过环境变量配置（根据用户参数设置）"
fi

# 8. 配置pip国内镜像源
echo "步骤 8/8: 配置pip镜像源..."
mkdir -p ~/.pip
...
echo "安装目录: ${PYTHON_INSTALL_DIR}"
echo "二进制路径: ${PYTHON_INSTALL_DIR}/bin"
echo "已添加到环境变量: ${ADD_TO_PATH}"

if [ "${ADD_TO_PATH}" = "true" ]; then
    echo "环境变量文件: /etc/profile.d/${PYTHON_DIR_NAME}.sh"
    echo ""
    echo "=== 使用说明 ==="
    echo "1. 立即生效: source /etc/profile.d/${PYTHON_DIR_NAME}.sh"
    echo "2. 直接调用: python${PYTHON_VERSION%.*} 或 pip${PYTHON_VERSION%.*}"
else
    echo ""
    echo "=== 使用说明 ==="
    echo "1. 使用完整路径调用: ${PYTHON_INSTALL_DIR}/bin/python${PYTHON_VERSION%.*}"
    echo "2. 手动临时添加环境变量: export PATH=${PYTHON_INSTALL_DIR}/bin:\$PATH"
fi

echo ""
echo "=== 多版本管理 ==="
echo "要安装其他版本，重新运行此脚本并指定不同版本号即可"
echo "示例: $0 -v 3.11.5 -a  # 安装Python 3.11.5并添加到环境变量"
